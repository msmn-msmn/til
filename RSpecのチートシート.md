...
#### 　EX.`itの部分をfitまたはxitとする`ことで実行するテスト限定することが出来ます。

　　fit：fitのブロックのみテストコードを実行する。itのブロックは実行されない。

```
          describe "クラス名やメソッドの処理内容などを記載する" do
          context "状況や条件などを記載する" do
            fit "テストケースの説明" do            ←itの部分をfitにする
              ...
            end
          end
        end
...
　　xit：xitのブロックのテストコードは実行しない。fit又はitのブロックが実行される。
```
          describe "クラス名やメソッドの処理内容などを記載する" do
          context "状況や条件などを記載する" do
            xit "テストケースの説明" do            ←itの部分をxitにする
              ...
            end
          end
        end
